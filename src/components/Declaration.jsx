...
const Declaration = ({ closeModal }) => {
  ...
  const handleProceed = () => {
    if (agreed) {
      ...
      navigate("/register"); // Then go to register
    }
  };
  ...
  return (
    <div>
      ...
      <div className="flex justify-end space-x-4">
        <button
          onClick={closeModal}
          className="border border-pink-500 text-pink-600 px-6 py-2 rounded-md hover:bg-pink-50"
        >
          Close
        </button>
        <button
          ...
          className={`px-6 py-2 rounded-md text-white ${
            agreed ? "bg-pink-600 hover:bg-pink-700" : "bg-pink-300 cursor-not-allowed"
          }`}
        >
          Proceed
        </button>
